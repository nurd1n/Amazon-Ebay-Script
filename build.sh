if [ $(echo "$(cat imageamazon.py)" | sed 's/asin2/asin1/g' | python - |wc -l) -gt "0" ]; then
 echo "$(cat titleamazon.py)" | sed 's/asin2/asin1/g' | python - | tr -cd '[[:alnum:]] .,&:' | sed 's/.*/\L&/; s/[a-z]*/\u&/g' > deletetitleamazon.txt
 echo "echo \"\$(cat urlebay.py)\" | sed 's/keyword/$(cat deletetitleamazon.txt | tr -cd '[[:alnum:]] ' | sed 's/ /+/g')/g' | python -" | bash - | sed -n 1,5p > deleteebayurl.txt
...
echo "exiftool -Title=\"$(cat deletetitleamazon.txt)\" -Author=\"$(cat deleteauthor1.txt)\" -Description=\"The picture/image of $(cat deletetitleamazon.txt)\" -Keyword=\"$(cat deletetitleamazon.txt), $(cat deleteask.txt)\" -Copyright=\"Source http://www.domain.ekstension/wp-content/uploads/$(cat deletetitleamazon.txt | tr -cd '[[:alnum:]] ' | tr A-Z a-z | sed 's| |-|g').jpg\" $(cat deletetitleamazon.txt | tr -cd '[[:alnum:]] ' | tr A-Z a-z | sed 's| |-|g').jpg" | bash -
 #description
 echo "<h3>$(cat deletetitleamazon.txt)</h3><br/><div style=\"text-align: center;\"><img src=\"http://www.domain.ekstension/wp-content/uploads/$(cat deletetitleamazon.txt | tr -cd '[[:alnum:]] ' | tr A-Z a-z | sed 's| |-|g').jpg\" width=\"500\" heigh=\"auto\" alt=\"$(cat deletetitleamazon.txt)\" /><br/><br/><a $(printf "eyabayanes\ntulkanoli\njasunamikon\nkolpokloij\ngulanjuikolpo\npalasantoliong\ndongkelapolik\ndasmutyuki" | shuf -n 1)$(cat deletetitleamazon.txt | tr -cd '[[:alnum:]] ' | tr A-Z a-z | sed 's/ /+/g')\" title=\"Click this button to check product on ebay\" target=\"_blank\" class=\"fasc-button fasc-size-xlarge fasc-type-glossy fasc-rounded-medium fasc-ico-before dashicons-cart\" style=\"background-color: #db0d44; color: #ffffff;\" data-fasc-style=\"background-color:#db0d44;color:#ffffff;\">Check Product On Ebay</a></div><br/><div style=\"text-align: justify;\">$(cat deletedescriptionamazon.txt)<br/>$(cat deletedetailamazon.txt)<br/><br/><center><a $(printf "eyabayanes\ntulkanoli\njasunamikon\nkolpokloij\ngulanjuikolpo\npalasantoliong\ndongkelapolik\ndasmutyuki" | shuf -n 1)$(cat deletetitleamazon.txt | tr -cd '[[:alnum:]] ' | tr A-Z a-z | sed 's/ /+/g')\" title=\"Click this button to check best price on ebay\" target=\"_blank\" class=\"fasc-button fasc-size-xlarge fasc-type-glossy fasc-rounded-medium fasc-ico-before dashicons-cart\" style=\"background-color: #db0d44; color: #ffffff;\" data-fasc-style=\"background-color:#db0d44;color:#ffffff;\">Check Best Price On Ebay</a></center><br/>$(cat deletegalleryebay2.txt)<br/>$(cat deletereviewamazon.txt)<br/>$(cat deleteask.txt)<br/><center><a $(printf "eyabayanes\ntulkanoli\njasunamikon\nkolpokloij\ngulanjuikolpo\npalasantoliong\ndongkelapolik\ndasmutyuki" | shuf -n 1)$(cat deletetitleamazon.txt | tr -cd '[[:alnum:]] ' | tr A-Z a-z | sed 's/ /+/g')\" title=\"Click this button to buy on ebay\" target=\"_blank\" class=\"fasc-button fasc-size-xlarge fasc-type-glossy fasc-rounded-medium fasc-ico-before dashicons-cart\" style=\"background-color: #db0d44; color: #ffffff;\" data-fasc-style=\"background-color:#db0d44;color:#ffffff;\">Buy Now On Ebay</a></center></div>" | sed -e 's|http://ecx.images-amazon.com/images/|ajananage|g' > deletedescription.txt
 #upload youtube
 echo "wget -qO- --header=\"Accept: text/html\" --user-agent=\"Googlebot-Image/1.0\" \"http://www.bing.com/videos/search?scope=video&pq=$(cat deletetitleamazon.txt | tr -cd '[[:alnum:]] ' | tr A-Z a-z | sed 's/ /+/g')&sc=1-30&sp=-1&sk=&q=$(cat deletetitleamazon.txt | tr -cd '[[:alnum:]] ' | tr A-Z a-z | sed 's/ /+/g')&qft=+filterui:msite-youtube.com\" | grep -o '<a href=['\"'\"'\"][^\"'\"'\"']*['\"'\"'\"]' | grep 'watch?v' | sed -e 's/^<a href=[\"'\"'\"']//' -e 's/[\"'\"'\"']$//' -e 's/https:\\/\\/www.youtube.com\\/watch?v=//g' | shuf -n 1 > deleteyoutube.txt" | bash -
