...
echo "$(cat data/id.txt | shuf -n 1)" > deleteidpost.txt
 # upload image
cd /home/www/domain
echo "wp media import $(cat /home/ebay/attachment/deletetitleamazon.txt | tr -cd '[[:alnum:]] ' | tr A-Z a-z | sed 's| |-|g').jpg --post_id=$(cat /home/ebay/attachment/deleteidpost.txt | shuf -n 1) --porcelain --allow-root > /home/ebay/attachment/deleteid.txt" | bash -
 # sql file 2
